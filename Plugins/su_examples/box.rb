...
def self.create_box
    ...
    # and default values.
    prompts = [$exStrings.GetString("Width"), $exStrings.GetString("Height"),
               $exStrings.GetString("Depth")]
    values = [6.feet, 5.feet, 4.feet]
    
    # Note that for the default values, we specify the values like "6.feet"
    # This says that the value is Length, and tells what the units of
    ...
    return if not results # This means that the user canceled the operation
    ...
    # You could use a similar technique to create the other faces of
    ...
    # of the front of the face.  In order to control the direction and
...
#-----------------------------------------------------------------------------
file_loaded("su_examples/box.rb")
